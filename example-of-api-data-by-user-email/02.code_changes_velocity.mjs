...
/*
 * GitHub Code Velocity Analyzer CLI
 *
 * JSON Report Structure:
 * {
 *   commits: [{ sha, author, date, additions, deletions, totalChanges, ... }],
 *   summary: { totalCommits, totalAdditions, totalDeletions, velocity, ... },
 *   statistics: { commitSizes, byAuthor, byWeek, byMonth, ... },
 *   trends: { weeklyVelocity, monthlyVelocity, authorProductivity, ... }
 * }
 *
 * Use Cases:
  ...
  async makeRequest(url, retryCount = 0) {
    try {
      ...
      if (response.status === 401) {
        throw new Error(
          "Authentication failed. Please check your GitHub token and ensure it uses Bearer format. Token might lack proper repository access scopes."
        );
      }
      ...
      if (error.message.includes("401")) {
        throw new Error(
          "Authentication Format: GitHub API now requires Bearer token format. Please check your token permissions and scopes."
        );
      }
      if (error.message.includes("403")) {
        throw new Error(
          "API Endpoint: The request might be hitting rate limits. Token might lack proper repository access scopes."
        );
      }
  ...
  async analyzeCodeVelocity(repo, owner, startDate, endDate, token = null) {
    ...
    for (let i = 0; i < commits.length; i++) {
      ...
      try {
        ...
        // Track by author (using email as requested)
        const authorKey = commitData.authorEmail;
        if (!authorStats[authorKey]) {
          authorStats[authorKey] = {
            name: commitData.author,
            login: commitData.authorLogin,
    ...
    Object.entries(authorStats).forEach(([authorEmail, data]) => {
      ...
      analysisData.statistics.byAuthor[authorEmail] = {
        name: data.name,
        login: data.login,
        email: data.email,
...
Examples:
  ...
  node main.mjs -r owner/repo -l infinite -v  # Fetch all commits with no limit
...
Report Features:
  ...
  - Velocity analysis by user email (as requested)
  ...
  - Configurable fetch limits (200 default, infinite option)

Authentication:
  - Uses Bearer token format (GitHub API requirement)
  - Handles token permission errors with friendly messages
  - Automatic retry logic for failed requests
  - Rate limit handling and waiting

Error Handling:
  - Authentication Format: Uses Bearer token format instead of legacy format
  - API Endpoint: Handles rate limits and incorrect headers gracefully
  - Token Permissions: Clear error messages for token scope issues
`);
}
...
async function main() {
  ...
  try {
    ...
    // Show top authors by commit count (by email as requested)
    const topAuthors = Object.entries(analysisData.statistics.byAuthor)
      .sort(([, a], [, b]) => b.commitCount - a.commitCount)
      .slice(0, 5);

    if (topAuthors.length > 0) {
      console.log("👥 Top Contributors by Velocity (by email):");
      topAuthors.forEach(([email, stats], index) => {
