...
class GitHubAnalyzer {
   ...
   * @param {number} commitLimit - Maximum commits to analyze (default: 200, 0 for infinite)
  ...
  async analyzeRepository(
    ...
    // Set default dates if not provided (-2 days to now)
    if (!startDate) {
    ...
    console.log(
      `📊 Commit limit: ${commitLimit === 0 ? "Infinite" : commitLimit}`
    );
    ...
    try {
      ...
      if (commits.length === 0) {
        console.log(
          `⚠️  No commits found in date range ${startDate} to ${endDate}`
        );
      ...
      const report = {
        ...
        summary: {
          ...
          commitLimit: commitLimit === 0 ? "Infinite" : commitLimit,
          commitLimitReached: commitLimit > 0 && commits.length >= commitLimit,
        },
      ...
      if (commitLimit > 0 && commits.length >= commitLimit) {
        console.log(
          `⚠️  Commit limit reached (${commitLimit}). Use --limit 0 for infinite.`
        );
      }
  ...
  generateEmptyReport(repo, owner, startDate, endDate, commitLimit) {
    return {
      repository: {
        name: repo,
        owner: owner,
  ...
  async fetchCommitsInDateRange(
    ...
    while (hasMore && (commitLimit === 0 || allCommits.length < commitLimit)) {
      ...
      // Show progress
      const limitText = commitLimit === 0 ? "infinite" : commitLimit;
      ...
      if (commits.length === 0) {
        ...
        if (commitLimit > 0) {
          const remainingSlots = commitLimit - allCommits.length;
          if (remainingSlots <= 0) {
            break;
          }
        ...
        if (commitLimit > 0 && allCommits.length >= commitLimit) {
          hasMore = false;
        }
      }
    }
  ...
  generatePunchCardAnalysis(contributorData, commits) {
    ...
    contributors.forEach((contributor) => {
      contributor.punchCard.hourlyDistribution.forEach((count, hour) => {
        repoHourly[hour] += count;
      });

      contributor.punchCard.dailyDistribution.forEach((count, day) => {
        repoDaily[day] += count;
      });
  ...
  analyzeShiftPreferences(contributors) {
    ...
    contributors.forEach((contributor) => {
      const shifts = contributor.punchCard.shiftBreakdown;
      const totalCommits = shifts.morning + shifts.afternoon + shifts.evening;

      if (totalCommits > 0) {
        const morningPct = (shifts.morning / totalCommits) * 100;
        const afternoonPct = (shifts.afternoon / totalCommits) * 100;
        const eveningPct = (shifts.evening / totalCommits) * 100;
      ...
      try {
        ...
        if (!response.ok) {
          if (response.status === 401) {
            throw new Error(
              "Authentication failed. Please check your GitHub token format and permissions. The GitHub API now requires Bearer token format instead of the legacy token format."
            );
          } else if (response.status === 403) {
            const errorBody = await response.text();
            if (errorBody.includes("rate limit")) {
              throw new Error(
                "Rate limit exceeded. The request might be hitting rate limits. Please wait before retrying."
              );
            } else {
              throw new Error(
                "Access forbidden. The token might lack proper repository access scopes. Please ensure your token has the required permissions for this repository."
              );
            }
          } else if (response.status === 404) {
            throw new Error(
              "Repository not found or access denied. Check repository name and ensure your token has access to this repository."
            );
          } else {
            const errorBody = await response.text();
            throw new Error(
...
class GitHubAnalyzerCLI {
  ...
  parseArguments() {
    ...
    const options = {
      ...
      limit: 200, // Default commit limit
    };
      ...
      switch (arg) {
        ...
        case "--fetchLimit":
          ...
          if (isNaN(limitValue) || limitValue < 0) {
            throw new Error(
              "Limit must be a positive number or 0 for infinite"
            );
          }
...
Options:
  ...
  -l, --limit <number>        Commit limit (default: 200, use 0 for infinite)
...
Features:
  ...
  • Date Filtering: Filter results by date ranges where applicable
...
Examples:
  ...
  node main.mjs -r facebook/react --format csv --limit 500
  node main.mjs -r owner/repo --limit 0 --verbose  # Analyze all commits
  node main.mjs -r owner/repo --token ghp_xxxxxxxxxxxx --limit 100
...
Environment Variables:
  ...
  generateOutputFilename(owner, repo, format, start, end, limit) {
    ...
    const limitSuffix = limit === 0 ? "_unlimited" : `_limit${limit}`;
  ...
  convertToCSV(data) {
    ...
    const rows = data.contributors.map((contributor) => {
      const percentage =
        totalCommits > 0
          ? Math.round((contributor.commitCount / totalCommits) * 100)
          : 0;

      // Find peak hour and day for this contributor
      const peakHour = contributor.punchCard.hourlyDistribution.indexOf(
        Math.max(...contributor.punchCard.hourlyDistribution)
      );
      const peakDay = contributor.punchCard.dailyDistribution.indexOf(
        Math.max(...contributor.punchCard.dailyDistribution)
      );
  ...
  async run() {
    try {
      ...
      if (options.verbose) {
        ...
        console.log(`   End Date: ${options.end || "now (default)"}`);
        console.log(
          `   Commit Limit: ${options.limit === 0 ? "Infinite" : options.limit}`
        );
        console.log(
      ...
      console.log(
        `   Top Contributor: ${
          report.insights.topContributorByCommits || "None"
        }`
      );
      ...
      if (options.verbose && report.contributors.length > 0) {
        ...
        report.contributors.slice(0, 3).forEach((contributor, index) => {
          const dominantShift = Object.entries(
            contributor.punchCard.shiftBreakdown
        ...
        console.log(
          `   Morning People: ${shiftPrefs.morning?.length || 0} contributors`
        );
        console.log(
          `   Afternoon People: ${
            shiftPrefs.afternoon?.length || 0
          } contributors`
        );
        console.log(
          `   Evening People: ${shiftPrefs.evening?.length || 0} contributors`
        );
      }
    } catch (error) {
      console.error("\n❌ Error:", error.message);
