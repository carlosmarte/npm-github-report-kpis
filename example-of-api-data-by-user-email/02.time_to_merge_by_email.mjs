...
class GitHubAnalyzer {
  ...
  async fetchUserEmail(username) {
    ...
    try {
      ...
      // GitHub API returns email only if user has made it public
      const email = user.email || `${username}@users.noreply.github.com`;

      this.userCache.set(username, email);
      this.log(`Fetched email for ${username}: ${email}`, "debug");
      return email;
    } catch (error) {
      this.log(
      ...
      this.userCache.set(username, fallbackEmail);
  ...
  /**
   * Analyze time-to-merge per PR breakdown by user email
   * @param {string} repo - Repository name
   * @param {string} owner - Repository owner
   * @param {string} startDate - Start date (YYYY-MM-DD)
   * @param {string} endDate - End date (YYYY-MM-DD)
   * @param {string} token - GitHub token (optional, uses instance token if not provided)
   * @returns {Object} Analysis data with time-to-merge breakdown by user email
   */
  async analyzeTimeToMergeByUserEmail(
    repo,
    owner,
    startDate,
    endDate,
    ...
    // Collect unique users first for email fetching
    ...
    this.log(
      `Fetching email addresses for ${uniqueUsers.size} unique users...`,
      "verbose"
    );
    ...
    for (const username of uniqueUsers) {
      this.showProgress(
        ++emailFetchCount,
        uniqueUsers.size,
        `Fetching email for ${username}`
      );
      const email = await this.fetchUserEmail(username);
      userEmailMap.set(username, email);
    }
    ...
    for (let i = 0; i < filteredPRs.length; i++) {
      ...
      const authorEmail = userEmailMap.get(pr.user.login);
      ...
      if (pr.merged_at) {
        ...
        // Track by user email
        if (!userEmailStats[authorEmail]) {
          userEmailStats[authorEmail] = {
            mergeTimes: [],
            count: 0,
        ...
        userEmailStats[authorEmail].mergeTimes.push(timeToMerge.totalHours);
        userEmailStats[authorEmail].count++;
        userEmailStats[authorEmail].prs.push({
          number: pr.number,
          title: pr.title,
    ...
    // Calculate user email statistics
    Object.entries(userEmailStats).forEach(([email, data]) => {
  ...
  async exportToCSV(data, filename) {
    try {
      ...
      // Add user email breakdown section
      if (Object.keys(data.statistics.byUserEmail).length > 0) {
        statsSection.push("# USER EMAIL BREAKDOWN", "");
        Object.entries(data.statistics.byUserEmail)
          .sort(([, a], [, b]) => b.totalMergedPRs - a.totalMergedPRs)
          .forEach(([email, stats]) => {
            statsSection.push(
              `# ${email} (${stats.username}): ${
                stats.totalMergedPRs
              } PRs, avg: ${this.formatDuration(stats.mean)}`
            );
          });
        statsSection.push("");
      }
...
function showHelp() {
  console.log(`
GitHub Repository Analysis Tool - Time-to-Merge per PR Breakdown by User Email
...
Report Features:
  ...
  - Breakdown by user email addresses (fetched from GitHub API)
  - Statistical analysis (mean, median, percentiles) per user
  - Individual PR details per user email
  - Monthly trends and overall statistics
  - Export to JSON or CSV formats with embedded date ranges
  - Progress tracking and retry logic for API reliability

Metrics Included:
  - Individual PR merge times per user email
  - Statistical distribution of merge times per user
  - User email to username mapping
  - Complete PR metadata (size, complexity indicators)
  - Monthly aggregation trends

Note: User emails are fetched from GitHub API. If a user hasn't made their email 
public, a fallback email format (username@users.noreply.github.com) is used.
`);
}
...
function generateFilename(repo, format, dateRange) {
  ...
  const dateRangeStr = dateRange
    ...
    : "";
  return `github_time_to_merge_by_email_${repoName}${dateRangeStr}_${timestamp}.${format}`;
}
...
async function main() {
  ...
  try {
    ...
    // Display summary
    console.log("\n📊 Time-to-Merge Analysis Summary (By User Email):");
    ...
    // Show top contributors by merge count (by email)
    const topContributors = Object.entries(analysisData.statistics.byUserEmail)
      .sort(([, a], [, b]) => b.totalMergedPRs - a.totalMergedPRs)
      .slice(0, 5);
