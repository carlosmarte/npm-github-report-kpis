...
class GitHubAnalyzer {
  ...
  async fetchCommits() {
    ...
    for (const branch of possibleBranches) {
      try {
        const testUrl = `${this.baseUrl}/repos/${this.owner}/${this.repo}/commits?sha=${branch}&per_page=1`;
        await this.makeRequest(testUrl);
        workingBranch = branch;
        this.log(`Using branch: ${branch}`, "verbose");
        break;
      } catch (error) {
        this.log(`Branch ${branch} not found, trying next...`, "debug");
        continue;
      }
    }
    ...
    if (!workingBranch) {
      throw new Error(
        "Could not find main, master, or develop branch in repository"
      );
    }

    let url = `${this.baseUrl}/repos/${this.owner}/${this.repo}/commits?sha=${workingBranch}&per_page=100`;
    ...
    progressBar.complete();
    this.log(
      `Successfully fetched ${allCommits.length} commits from ${workingBranch} branch`
    );
    return allCommits;
  }
  ...
  async analyzeRepository() {
    ...
    // Fetch all commits from main branch
  ...
  generateReport() {
    ...
    const report = {
      metadata: {
        ...
        requestedDateRange: {
          start: this.startDate || "beginning",
      ...
      commitTimeline: this.commits, // Tree structure based on commit order
      fileMetadata: this.fileMetadata, // Root nodes with file metadata
    };
...
EXAMPLES:
  # Basic analysis with default 200 API request limit
...
REPORT FEATURES:
  📊 Commit Timeline Analysis    - Chronological view of all commits merged to main
  📁 File Change Tracking       - Detailed file modification history  
  🌳 Tree Structure View        - File metadata showing changes over time range
...
async function main() {
  try {
    ...
    if (options.start || options.end) {
      console.log(
        `📅 Date range: ${options.start || "beginning"} to ${
          options.end || "now"
        }`
      );
    }
    ...
    const authors = Object.entries(report.summary.authorActivity)
      .sort(([, a], [, b]) => b.commitCount - a.commitCount)
      .slice(0, 3);

    if (authors.length > 0) {
      console.log(`\n👥 Top contributors:`);
      authors.forEach(([author, stats], index) => {
        console.log(`   ${index + 1}. ${author}`);
        console.log(
          `      📝 ${stats.commitCount} commits, ${stats.uniqueFilesChanged} files, avg ${stats.averageChangesPerCommit} changes/commit`
        );
      });
    }
    ...
    console.log(
      `   The report includes commit timeline (tree structure) and file metadata for tracking changes over time.`
    );
  } catch (error) {
    console.error(`\n❌ Error: ${error.message}`);
